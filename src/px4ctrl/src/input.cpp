...
void RC_Data_t::feed(mavros_msgs::RCInConstPtr pMsg)
{
    ...
    reboot_cmd = ((double)msg.channels[70] - 1000.0) / 1000.0;
    ...
    //std::cout << " rc_data.is_command_mode = " << is_command_mode << std::endl;
}
...
{
    bool centered = fabs(ch[0]) < 1e-5 && fabs(ch[0]) < 1e-5 && fabs(ch[0]) < 1e-5 && fabs(ch[0]) < 1e-5;
    return centered;
}
...
#define VEL_IN_BODY
    ...
    if ( (now - last_clear_count_time).toSec() > 1.0 )
    {
        if ( one_min_count < 10 )
        {
            ROS_WARN("ODOM frequency seems lower than 10Hz, which is too low!");
        }
        one_min_count = 0;
...
void Command_Data_t::feed(quadrotor_msgs::PositionCommandConstPtr pMsg)
{
    ...
    //ROS_INFO_STREAM_THROTTLE(0.5, "j1=" << j.transpose());
