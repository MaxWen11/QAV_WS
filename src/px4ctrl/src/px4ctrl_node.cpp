...
int main(int argc, char *argv[])
{
    ...
    ros::Subscriber state_sub =
        nh.subscribe<mavros_msgs::State>("/drone1/mavros/state",
                                         10,
                                         boost::bind(&State_Data_t::feed, &fsm.state_data, _1));

    ros::Subscriber extended_state_sub =
        nh.subscribe<mavros_msgs::ExtendedState>("/drone1/mavros/extended_state",
                                                 10,
    ...
    ros::Subscriber imu_sub =
        nh.subscribe<sensor_msgs::Imu>("/drone1/mavros/imu/data", // Note: do NOT change it to /mavros/imu/data_raw !!!
                                       100,
    ...
    {
        rc_sub = nh.subscribe<mavros_msgs::RCIn>("/drone1/mavros/rc/in",
                                                 10,
                                                 boost::bind(&RC_Data_t::feed, &fsm.rc_data, _1));
    }

    ros::Subscriber bat_sub =
        nh.subscribe<sensor_msgs::BatteryState>("/drone1/mavros/battery",
                                                100,
    ...
    fsm.ctrl_FCU_pub = nh.advertise<mavros_msgs::AttitudeTarget>("/drone1/mavros/setpoint_raw/attitude", 10);
    fsm.traj_start_trigger_pub = nh.advertise<geometry_msgs::PoseStamped>("/traj_start_trigger", 10);

    fsm.debug_pub = nh.advertise<quadrotor_msgs::Px4ctrlDebug>("/drone1/debugPx4ctrl", 10); // debug

    fsm.set_FCU_mode_srv = nh.serviceClient<mavros_msgs::SetMode>("/drone1/mavros/set_mode");
    fsm.arming_client_srv = nh.serviceClient<mavros_msgs::CommandBool>("/drone1/mavros/cmd/arming");
    fsm.reboot_FCU_srv = nh.serviceClient<mavros_msgs::CommandLong>("/drone1/mavros/cmd/command");
