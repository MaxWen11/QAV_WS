...
public:
	...
	//Eigen::Vector3d manual_vel_sp;


		// 滤波器内部状态 (Filter States)
	Eigen::Vector3d filter_p; // 平滑后的位置
	Eigen::Vector3d filter_v; // 平滑后的速度
	Eigen::Vector3d filter_a; // 平滑后的加速度

	// 滤波参数 (Tuning Parameters)
	double time_const = 0.8; // 时间常数，越小响应越快（约等于0.1~0.5秒达到目标）
	double damp_ratio = 1.5; // 阻尼比，1.0为无超调
...
private:
	...
	bool is_recording = false; // 记录当前是否正在录制
    void manage_rosbag(bool start); // 辅助函数：开启或停止录制
};
