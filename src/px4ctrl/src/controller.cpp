    ...
    double voltage)
{
 ...
 // Check the given velocity is valid.
  ...
  // ESO (Extended State Observer) Implementation
  static Eigen::Vector3d eso_pos = Eigen::Vector3d::Zero();
  static Eigen::Vector3d eso_vel = Eigen::Vector3d::Zero();
  static Eigen::Vector3d eso_dist = Eigen::Vector3d::Zero();
  static ros::Time last_eso_time = ros::Time(0);
  
  // 初始推力设为重力，避免第一次进入时为0
  static double last_thrust_acc_norm_ = param.gra; 
  
  // [新增] 起飞标志位 (自锁开关)
  static bool has_taken_off = false; 

  ros::Time now = ros::Time::now();
  double dt = (now - last_eso_time).toSec();

  // 1. 安全保护：如果时间间隔异常(>1.0s)，强制重置一切状态
  // 这通常发生在程序刚启动、或者中间断连重启时
  if (dt <= 0 || dt > 1.0) {
    has_taken_off = false;
    dt = 0;
  }
  ...
  // 2. 起飞检测逻辑 (仅在未起飞时检测)
  // 条件：推力足够大(>3.0) 且 高度足够高(>0.5m)
  if (!has_taken_off) {
    if (last_thrust_acc_norm_ > 3.0 && odom.p(2) > 0.2) {
        has_taken_off = true;
        // 打印一次日志方便调试
        ROS_INFO("[px4ctrl] ESO Activated: Airborne latch set (Height > 0.5m).");
    }
  }

  // 3. 根据状态决定是 复位 还是 运行ESO
  if (!has_taken_off) {
    // --- 地面/未起飞状态：持续跟随里程计，扰动清零 ---
    eso_pos = odom.p;
    eso_vel = odom.v;
    eso_dist.setZero();
    last_thrust_acc_norm_ = param.gra; // 保持默认悬停推力

  } else {
    // --- 飞行状态：运行 ESO 核心算法 ---
    
    // ESO Update
    Eigen::Vector3d input_acc = odom.q * Eigen::Vector3d(0, 0, last_thrust_acc_norm_) + Gravity;
    Eigen::Vector3d next_dist;

    // =================================================================
    // SELECT ESO TYPE: 
    // false = Position-based (3rd Order), true = Velocity-based (2nd Order)
    const bool use_velocity_eso = true; 
    // =================================================================

    if (use_velocity_eso) {
        // Velocity-based ESO (2nd Order)
        // States: z1=vel, z2=dist_acc
        const double omega = 8.0; // Bandwidth (tunable, usually higher for Vel ESO)
        const double b1 = 2.0 * omega;
        const double b2 = omega * omega;

        Eigen::Vector3d vel_err = odom.v - eso_vel;
        eso_vel += (input_acc + eso_dist + b1 * vel_err) * dt;
        next_dist = eso_dist + (b2 * vel_err) * dt;
        
        eso_pos = odom.p; // Sync position for consistency
    } else {
        // Position-based ESO (3rd Order)
        // States: z1=pos, z2=vel, z3=dist_acc
        const double omega = 8.0; // Bandwidth (tunable)
        const double b1 = 3.0 * omega;
        const double b2 = 3.0 * omega * omega;
        const double b3 = omega * omega * omega;

        Eigen::Vector3d pos_err = odom.p - eso_pos;
        eso_pos += (eso_vel + b1 * pos_err) * dt;
        eso_vel += (input_acc + eso_dist + b2 * pos_err) * dt;
        next_dist = eso_dist + (b3 * pos_err) * dt;
    }
    ...
    Eigen::Vector2d dist_xy(next_dist(0), next_dist(1));

    if (dist_xy.norm() > limit_xy) dist_xy = dist_xy.normalized() * limit_xy;

    if (std::abs(next_dist(2)) > limit_z) next_dist(2) = (next_dist(2)>0 ? 1:-1) * limit_z;


    eso_dist = Eigen::Vector3d(dist_xy(0), dist_xy(1), next_dist(2));
  ...
  // Pass eso_dist as the 3rd argument to subtract it from the desired acceleration
  Eigen::Vector3d total_des_acc = computeLimitedTotalAcc(pid_error_accelerations, des.a, eso_dist);
  ...
  // 更新记录推力，供下一帧 ESO 使用
  last_thrust_acc_norm_ = debug.des_thr;
  ...
  debug.eso_vel_x = eso_vel(0);
  debug.eso_vel_y = eso_vel(1);
  debug.eso_vel_z = eso_vel(2);

  debug.real_x = odom.p(0);
  debug.real_y = odom.p(1);
  debug.real_z = odom.p(2);

  debug.real_vx = odom.v(0); 
  debug.real_vy = odom.v(1);
  debug.real_vz = odom.v(2);
